...
import CartItemInViewMode from './CartItemInViewMode';
const SearchCart = ({ products, setproducts, cartlist, setcartlist }) => {
    const [searchCartInput, setsearchCartInput] = useState('')
    function handleChange(event) {
        setsearchCartInput(event.target.value)
        ...
        products_tmp.map(function (product) {
            if (input != '') {
                ...
                if (Id_str.includes(input_lowercase) || titleLower.includes(input_lowercase) ||
                    ...
                    product.isvisibleinCart = true
                } else {
                    product.isvisibleinCart = false
                }
            } else {
                product.isvisibleinCart = true
            }
    ...
    return (
        <>
            <input className='search-input' placeholder='search' type={'text'}
                onChange={(e) => handleChange(e)} value={searchCartInput} />
            <p> {searchCartInput} </p>
